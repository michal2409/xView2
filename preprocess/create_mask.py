...
train_dirs = ['/scidatasm/michalf/train']
...
def process_image(json_file):
    ...
    cv2.imwrite(json_file.replace('/labels/', '/targets/').replace('_pre_disaster.json', '_pre_disaster.png'), msk, [cv2.IMWRITE_PNG_COMPRESSION, 9])
    cv2.imwrite(json_file.replace('/labels/', '/targets/').replace('_pre_disaster.json', '_post_disaster.png'), msk_damage, [cv2.IMWRITE_PNG_COMPRESSION, 9])
